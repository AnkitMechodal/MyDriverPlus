...
const VerifyYourScreenMobile = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const userId = route?.params?.itemEmailNumber;
                if (userId) {
    ...
    }, [route?.params?.itemEmailNumber]);


    const storeAccountId = async (storeid: any) => {
        try {
            await AsyncStorage.setItem('user_id_num', JSON.stringify(storeid));
            console.log('user_id_num===>', JSON.stringify(storeid));
        } catch (error) {
            // Handle any errors that might occur during the storage operation
            console.log('Error user_id_num :', error);
        }
    }
    ...
    const axiosPostSetDataGetInfo = async () => {
        ...
        const storedLinkedId = await AsyncStorage.getItem('user_id_num');
        if (storedLinkedId !== null) {
            const url = 'https://rideshareandcourier.graphiglow.in/api/email_detail/details';

            // Prepare data in JSON format
            const data = {
                mobilenumber: JSON.parse(storedLinkedId),
            };
            ...
            await axios.post(url, data, {
                ...
                .then(response => {
                    if (response.status === 200 &&
                        ...
                        user_mobilenumber = response?.data?.user?.mobilenumber;
                        user_email = response?.data?.user?.email;

                        // Set Status 
                        user_mobilenumberStatus =
                            response?.data?.user?.mobilenumberStatus;
                        console.log("user_mobilenumberStatus", user_mobilenumberStatus);

                        user_emailStatus =
                            response?.data?.user?.emailStatus;
...
export default VerifyYourScreenMobile;
